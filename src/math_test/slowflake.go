...
const (
	...
	epoch int64 = 1571970882000 // 这个是我在写epoch这个常量时的时间戳(毫秒)
)

func main() {
	fmt.Println(workerMax, numberMax, timeShift, workerShift, epoch)
	// 生成节点实例
	...
	// 成功生成 snowflake ID
	fmt.Println("All", count, "snowflake ID Get success!")
}
...
func (w *Worker) GetId() int64 {
	...
	if w.timestamp == now {
		...
		if w.number > numberMax {
			// 如果当前工作节点在1毫秒内生成的ID已经超过上限 需要等待1毫秒再继续生成一个
			// 不会有重复，因为number=0的情况，不会出现。number=4096的情况，
			// 去到了下一毫秒的number=0，同一个毫秒开始，number从1开始。所以同一个worker不会重复
			for now <= w.timestamp {
